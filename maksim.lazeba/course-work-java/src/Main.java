...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        List<TestCase> train = DatasetParser.getTrainSet(args[0], 4000);
        List<TestCase> test = DatasetParser.getTestSet(args[0], 1000);
        int threads = Math.min(Integer.valueOf(args[1]), 4);
        ExecutorService executor = Executors.newFixedThreadPool(threads);
        for (int i = 2; i < args.length; i++){
            executor.submit(new Trainer(args[i], train, test, "results"));
        }
    }
